...
parser = OptionParser()
parser.add_option(
    "-l", "--dma_list", dest="dma_list", help="list of all dma files", metavar="dma_list")
(options, args) = parser.parse_args()

if options.dma_list is None:
    dma_region_codes = ['ZWAL4801']
else:
    dma_region_codes = options.dma_list
            ...
            for table_name, bbox in pg_tables:
                ...
                try:
                    ...
                    try:
                        ...
                        get_map_url = '%s/%s/wms?service=WMS&version=1.1.0&request=GetMap&layers=%s:%s\
                        &bbox=%s,%s,%s,%s&width=%s&height=%s&srs=EPSG:%s&styles=&format=%s' % (
                    ...
                    try:
                        ...
                        for code, coordinates in dma_codes_list:
                            ...
                            bbox=%s,%s,%s,%s&width=%s&height=%s&srs=EPSG:%s&styles=&format=%s' % (
                                ...
                                max_x,
                                max_y,
                                self.geoserver_config.default['HEIGHT'], self.geoserver_config.default['WIDTH'], max_y,
                                self.geoserver_config.default['FORMAT'])
