...
#https://github.com/ztimol/digital-twin-refactor/blob/main/002_Twin/010_Build_One_Network_Graph/020_Spatial_Join_on_Points_to_Lines.py
point_layers = [
 'wNetworkMeter',
 'wChamber',
 'wNetworkOptValve',
...
def save_ehms(table, dataframe, mode='overwrite', debug=False):
    try:
        ...
        if debug: table_name += '_ref'
        properties = {}
        metastore = Metastore()
        metastore.save_table(dbutils, schema_name, table_name, dataframe, properties, mode=mode)
...
gdf_mains = gpd.read_file(shape_file)
...
def antijoin_points(left_on, right_on):
    ...
    tmp = pd.merge(left=left_on, right=right_on.set_index('Point'), how='left', left_index = True, right_on='Point', indicator=True)
...
def join_points_on_lines(points: gpd.GeoDataFrame, lines: gpd.GeoDataFrame, tol: float = None) -> gpd.GeoDataFrame:
    ...
    df.rename(columns={'index': 'Point', 'index_left' : 'Point', 'GISID': 'Point', 'index_right': 'Line'}, inplace=True)
    df = df[['Point', 'Line']]
...
def join_points_with_tolerance(layer: str, points: gpd.GeoDataFrame, lines: gpd.GeoDataFrame, tol: float) -> gpd.GeoDataFrame:
...
for layer in point_layers:
    ...
    if tolerances.get(layer):
        ...
        gdf_tol = join_points_with_tolerance(layer, gdf, gdf_mains, tol=tolerances.get(layer))
    ...
    if save:
        pdf = spark.createDataFrame(map_point_line, schema='Point long, Line long, layer string')
