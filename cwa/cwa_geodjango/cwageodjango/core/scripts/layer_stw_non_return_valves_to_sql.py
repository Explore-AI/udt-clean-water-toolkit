...
from cwageodjango.assets.models import NonReturnValve
from cwageodjango.utilities.models import DMA, Utility


class Command(BaseCommand):
    help = "Write Severn Trent Water Non Return Valve layer data to sql"
...
Large numbers of features will take a long time to save."""
        ...
        non_return_valve_layer = ds[layer_index]

        new_non_return_valves = []
        for feature in non_return_valve_layer:
            gid = feature.get("tag")
            geom = feature.geom
            geom_4326 = feature.get("wkt_geom_4326")

            new_non_return_valve = NonReturnValve(
                tag=gid, geometry=geom.wkt, geometry_4326=geom_4326
            )
            new_non_return_valves.append(new_non_return_valve)

            if len(new_non_return_valves) == 100000:
                NonReturnValve.objects.bulk_create(new_non_return_valves)
                new_non_return_valves = []

        # save the last set of data as it will probably be less than 100000
        if new_non_return_valves:
            NonReturnValve.objects.bulk_create(new_non_return_valves)

        # get the utility
        utility = Utility.objects.get(name="severn_trent_water")

        DMAThroughModel = NonReturnValve.dmas.through
        bulk_create_list = []
        for non_return_valve in NonReturnValve.objects.filter(dmas=None).only(
            "id", "geometry"
        ):
            wkt = non_return_valve.geometry.wkt
            ...
            bulk_create_list.extend(
                [
                    DMAThroughModel(nonreturnvalve_id=non_return_valve.pk, dma_id=dma_id)
