...
if [[ -z ${DB_CONTAINER_ID} ]];then
  ...
  docker run -it --rm --env-file ../../docker/env_files/.db_env --volume=docker_neo4j-data:/data --volume=docker_neo4j-backups:/var/lib/neo4j/backups/ neo4j:5.18.0-community-bullseye bin/neo4j-admin database dump neo4j --verbose --to-path=./backups/ --overwrite-destination=true
