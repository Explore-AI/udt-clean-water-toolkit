...
DB_CONTAINER_ID=`docker ps | grep udtneo4j | grep neo4j:5.18.0-community-bullseye | awk '{ print $1 }'`
...
while getopts ${OPTSTRING} opt; do
    case ${opt} in
        f)
            ...
            docker run -it --rm --env-file ../../docker/env_files/.db_env --volume=docker_neo4j-data:/data --volume=docker_neo4j-backups:/var/lib/neo4j/backups/ neo4j:5.18.0-community-bullseye bin/neo4j-admin database load neo4j --from-path=/var/lib/neo4j/backups/ --verbose --overwrite-destination=true
